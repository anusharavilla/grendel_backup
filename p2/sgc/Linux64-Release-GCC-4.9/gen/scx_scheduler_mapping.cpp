...
public:
    ...
    void set() {
        if (! request) {
            request = true;
            if (sg::thread_equal(owner->simulation_thread_id, sg::thread_self())) {

                // Called from SystemC thread...
                event.notify();
            } else {
    ...
    void clear() {
        ...
        sc_assert(sg::thread_equal(owner->simulation_thread_id, sg::thread_self()));
        event.cancel();
    }
    operator bool() const {
        return request;
...
public:
    ...
    void notify(sg::ticks_t t)
    {
        ...
        event.notify(owner->simulated_time_from_ticks(t));
    }
...
#if defined(SYSTEMC_2_3_1)
    SCX_REPORT_INFO_VERB(SCX_DEBUG,
                         "scx::scheduler_mapping",
                         "scheduler_mapping(): SystemC implementation has sc_core::sc_time::from_value(sc_dt::uint64)");
#elif defined(SYSTEMC_2_3_0)
    SCX_REPORT_INFO_VERB(SCX_DEBUG,
                         "scx::scheduler_mapping",
                         "scheduler_mapping(): SystemC implementation has sc_core::sc_time(sc_dt::uint64, bool)");
#else
    SCX_REPORT_WARNING(SCX_ID_FM_DEPRECATION, "This implementation of SystemC does not support constructing sc_core::sc_time from sc_dt::uint64, simulation speed might be impacted");
#endif  /* SYSTEMC_2_3_1/0 */
...
void scheduler_mapping::wait(sg::ticks_t ticks)
{
    ...
    if_verbose(V_SYSTEMC)
        SCX_REPORT_INFO_VERB(SCX_DEBUG, "scx::scheduler_mapping", "wait(): sc_core::wait(%llu)", timeout.value());
...
void scheduler_mapping::addSynchronisationPoint(sg::ticks_t ticks)
{
#if ! (defined(IEEE_1666_SYSTEMC) && (IEEE_1666_SYSTEMC >= 201101L))
...
#endif  /* IEEE_1666_SYSTEMC */
    if_verbose(V_OTHER)
        SCX_REPORT_INFO_VERB(SCX_DEBUG, "scx::scheduler_mapping", "getNextSyncPoint(): %llu", q.value());
    return (q.value());
}
