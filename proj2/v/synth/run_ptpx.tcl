...
if { $type eq "unrouted" } then {
  read_verilog "${GATE_DIR}AHB_Lite_2s_final.v"
  read_verilog "${GATE_DIR}CORTEXM0DS_final.v"
  read_verilog "${GATE_DIR}dma_unit_final.v"
  read_verilog "${GATE_DIR}DW_memctl.v"
  read_verilog "${GATE_DIR}integrated_cpu_dma.v"
} else {
