...
{
public:
  bool init_val; // initial condition flag 
...
public:
  ...
  sc_core::sc_event same_time_event;// adding events for synchronization
  SC_HAS_PROCESS(SimpleBusLT);
  SimpleBusLT(sc_core::sc_module_name name) :
    sc_core::sc_module(name)
  {
      init_val = false; // setting initial value to be false 
  ...
  {  
     // if(init_val==0)
     // { same_time_event[1].notify(sc_core::SC_ZERO_TIME);}
   // init_val += 1; 
    if(init_val!=false)
    { //cout<<"when init_val is not 1"<<init_val<<"\n";
       wait(same_time_event); }//cout<<"init_val when not 0"<<init_val<<"\n";  } // wait if busy
    //else cout<<"when init_val is 0"<<init_val<<"\n"; 
      init_val=true;
    ...
    same_time_event.notify(sc_core::SC_ZERO_TIME); // notify that the event has been processed
    init_val =false ;
    //cout<<"notify"<<sc_core::sc_time_stamp()<<"\t"<<init_val<<"\n";
