...
/** \file
 * Class definition for protocol Cortex__PVBus.
 */

#ifndef Cortex__PVBus_ClassDef_h_
#define Cortex__PVBus_ClassDef_h_
...
public:
    ...
    void SetPort(Cortex__PVBus *port) { thisPort = port; }
...
private:
    ...
    Cortex__PVBus *thisPort;
};


// Protocol (port) class definition
class Cortex__PVBus: public sg::Port
{
private:
...
// Mapping object
// (these ports are implemented inside Cortex__PVBus__AddressablePort)
class Cortex__PVBus__AddressablePort_OutPort: public Cortex__PVBus
{
public:
    // constructor
    Cortex__PVBus__AddressablePort_OutPort(uint32_t inFirst,
                                                 uint32_t inLast,
                                                 uint32_t outFirst,
                                                 uint32_t outLast):
    inFirst__(inFirst),
...
class Cortex__PVBus__AddressablePort: public Cortex__PVBus
{
public:
    ...
    Cortex__PVBus__AddressablePort():
    defaultOutPort__(0),
    portName__("(unknown name)")
    {
        ...
        bind_method(*this, mapRange, &Cortex__PVBus__AddressablePort::mapRange__, this);
        bind_method(*this, mapDefault, &Cortex__PVBus__AddressablePort::mapDefault__, this);
        bind_method(*this, unmapRange, &Cortex__PVBus__AddressablePort::unmapRange__, this);
        bind_method(*this, isMapped, &Cortex__PVBus__AddressablePort::isMapped__, this);
    ...
    Cortex__PVBus__AddressablePort_OutPort *locateOutPort__(uint32_t &addr)
    {
        OutPortList__::iterator it;
        for (it = outPortList__.begin(); it != outPortList__.end(); ++it)
        {
    ...
    {
        Cortex__PVBus__AddressablePort_OutPort *masterPort =
            new Cortex__PVBus__AddressablePort_OutPort(inFirst, inLast, outFirst, outLast);
    ...
    {
        delete defaultOutPort__;
        defaultOutPort__ = new Cortex__PVBus__AddressablePort_OutPort(0, uint32_t(0) - 1, 0, uint32_t(0) - 1);
        return defaultOutPort__;
    }

    void unmapRange__(sg::Port* port)
    {
        outPortList__.remove((Cortex__PVBus__AddressablePort_OutPort*)port);
...
private:
    typedef std::list<Cortex__PVBus__AddressablePort_OutPort*> OutPortList__;
    OutPortList__ outPortList__;
    Cortex__PVBus__AddressablePort_OutPort *defaultOutPort__;
